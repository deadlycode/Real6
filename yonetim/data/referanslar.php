...
if (!empty($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") {
    ...
    $request = $_REQUEST;
    $col = ["id", "sira", "adi", "anasayfa", "durum", "islem"];
    $sql = "SELECT * FROM referanslar WHERE dil = '" . $_SESSION["admin_dil"] . "'";
    ...
    foreach ($islem as $row) {
        ...
        if ($row["resim"] == "") {
            ...
            $resim = "href=\"../" . tema . "/uploads/referanslar/" . $row["resim"] . "\" class=\"btn btn-inverse-success btn-sm\" title=\"Resmi Göster\"";
        }
        $subdata = [];
        ...
        $subdata[] = $row["id"];
        $subdata[] = "<a href=\"referans-duzenle/" . $row["id"] . "" . $html . "\" class=\"renk_baslik\" title=\"Düzenle\">" . $row["adi"] . "</a>";
        $subdata[] = $anasayfa;
        $subdata[] = $durum;
        $subdata[] = "<a " . $resim . "><i class=\"ti-image\"></i></a>\r\r\n\t\t\t\t\t<a href=\"referans-duzenle/" . $row["id"] . "" . $html . "\" class=\"btn btn-inverse-primary btn-sm\"><i class=\"ti-pencil-alt\" title=\"Düzenle\"></i></a>\r\r\n\t\t\t\t\t<a href=\"../_class/yonetim_islem.php?referanssil=ok&id=" . $row["id"] . "\" class=\"btn btn-inverse-danger btn-sm popconfirm\" data-original-title=\"\" title=\"Sil\"><i class=\"ti-trash\"></i></a>";
        $data[] = $subdata;
    }

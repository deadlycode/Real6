...
if (!empty($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") {
    ...
    $request = $_REQUEST;
    $uyeid = $_POST["uyeId"];
    $col = ["id", "sira", "adi", "ekleyen", "kategori", "yeni", "anasayfa", "durum", "islem"];
    $sql = "SELECT * FROM emlaklar WHERE dil = '" . $_SESSION["admin_dil"] . "' and uye = '1' AND uyeid = '" . $uyeid . "'";
    ...
    foreach ($islem as $row) {
        ...
        $uyebul = $db->query("SELECT * FROM uyeler WHERE id = '" . $row["ekleyen"] . "'")->fetch(PDO::FETCH_ASSOC);
        $subdata = [];
        ...
        $subdata[] = "<a href=\"musteri-duzenle/" . $uyebul["id"] . ".html\" title=\"Üye Detay\" target=\"_blank\">" . $uyebul["adsoyad"] . "</a>";
        $subdata[] = $kategori;

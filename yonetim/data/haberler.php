...
if (!empty($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") {
    ...
    $request = $_REQUEST;
    $col = ["id", "sira", "adi", "id", "anasayfa", "durum", "islem"];
    $sql = "SELECT * FROM haberler WHERE dil = '" . $_SESSION["admin_dil"] . "'";
    ...
    foreach ($islem as $row) {
        $haberfotosayisi = $db->query("SELECT * FROM haberfoto WHERE resimid = '" . $row["id"] . "'")->rowCount();
        if ($row["durum"] == 0) {
        ...
        if ($row["resim"] == "") {
            ...
            $resim = "href=\"../" . tema . "/uploads/haberler/" . $row["resim"] . "\" class=\"btn btn-inverse-success btn-sm\" title=\"Resmi Göster\"";
        }
        $subdata = [];
        ...
        $subdata[] = $row["id"];
        $subdata[] = "<a href=\"haber-duzenle/" . $row["id"] . "" . $html . "\" class=\"renk_baslik\" title=\"Düzenle\">" . $row["adi"] . "</a>";
        $subdata[] = "<a href=\"haber-fotograflar/" . $row["id"] . "" . $html . "\" class=\"btn btn-inverse-success btn-sm\" title=\"Fotoğrafları Gör\"><i class=\"fa fa-angle-right\"></i> Fotoğrafları Gör [" . $haberfotosayisi . "]</a>";
        $subdata[] = $anasayfa;
        $subdata[] = $durum;
        $subdata[] = "<a " . $resim . "><i class=\"ti-image\"></i></a>\r\r\n\t\t\t\t\t<a href=\"haber-duzenle/" . $row["id"] . "" . $html . "\" class=\"btn btn-inverse-primary btn-sm\" title=\"Düzenle\"><i class=\"ti-pencil-alt\"></i></a>\r\r\n\t\t\t\t\t<a href=\"../_class/yonetim_islem.php?habersil=ok&id=" . $row["id"] . "\" class=\"btn btn-inverse-danger btn-sm popconfirm\" title=\"Haber Sil\"><i class=\"ti-trash\"></i></a>";
        $data[] = $subdata;
    }

...
if (!empty($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") {
    ...
    $sql = "SELECT * FROM hizmetler WHERE dil = '" . $_SESSION["admin_dil"] . "'";
    ...
    foreach ($islem as $row) {
        ...
        if ($row["resim"] == "") {
            ...
            $resim = "href=\"../" . tema . "/uploads/hizmetler/" . $row["resim"] . "\" class=\"btn btn-inverse-success btn-sm\" title=\"Resmi Göster\"";
        }
        $subdata = [];
        ...
        $subdata[] = $row["id"];
        $subdata[] = "<a href=\"hizmet-duzenle/" . $row["id"] . "" . $html . "\" class=\"renk_baslik\" title=\"Düzenle\">" . $row["adi"] . "</a>";
        $subdata[] = $durum;
        $subdata[] = "<a " . $resim . "><i class=\"ti-image\"></i></a>\r\r\n\t\t\t\t\t<a href=\"hizmet-duzenle/" . $row["id"] . "" . $html . "\" class=\"btn btn-inverse-primary btn-sm\"><i class=\"ti-pencil-alt\" title=\"Düzenle\"></i></a>\r\r\n\t\t\t\t\t<a href=\"../_class/yonetim_islem.php?hizmetsil=ok&id=" . $row["id"] . "\" class=\"btn btn-inverse-danger btn-sm popconfirm\" data-original-title=\"\" title=\"Sil\"><i class=\"ti-trash\"></i></a>";
        $data[] = $subdata;
    }
